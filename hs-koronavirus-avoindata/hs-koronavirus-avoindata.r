...
for (i in 1:length(processedThlData$confirmed)) {
  ...
  par(mfrow=c(2,1))
  plot(y[1:(length(y)-3)],type="l",lwd=2,ylab="Korona tapauksia",xlab="P�ivi� 2020 alusta",main=title2,ylim=yl)
  abline(v=c((length(y)-3-28),(length(y)-3)),lwd=2)
  plot(y[(length(y)-3-28):(length(y)-3)],type="b",lwd=2,ylab="Korona tapauksia",xlab="Viimeiset 28 p�iv�� (poislukien uusimmat 4)",ylim=yl, new=FALSE)
